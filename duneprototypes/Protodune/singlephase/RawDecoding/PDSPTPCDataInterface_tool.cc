                                       ...
                                       std::vector<raw::RDStatus>& rdstatuses)
{
  ...
  apalist.push_back(-1);
  int retcode =
    retrieveDataAPAListWithLabels(evt, inputLabel, raw_digits, rd_timestamps, rdstatuses, apalist);
                                                       ...
                                                       std::vector<int>& apalist)
{
  ...
  for (size_t i = 0; i < apalist.size(); ++i) {
    ...
    if (lli == _input_labels_by_apa.end()) {
      MF_LOG_WARNING("PDSPTPCDataInterface:")
        << " No list of input labels known for APA " << apalist.at(i) << " Returning no data.";
    }
    for (size_t j = 0; j < lli->second.size(); ++j) {
      int retcode = retrieveDataAPAListWithLabels(
        evt, lli->second.at(j), raw_digits, rd_timestamps, rdstatuses, apalist);
...
int PDSPTPCDataInterface::retrieveDataAPAListWithLabels(
  art::Event& evt,
  std::string inputLabel,
  std::vector<raw::RawDigit>& raw_digits,
  std::vector<raw::RDTimeStamp>& rd_timestamps,
  ...
  std::vector<int>& apalist)
{
  ...
  _KeptCorruptData =
    false; // true if we identify a corruption candidate but are skipping the test to drop it
  ...
  if (inputLabel.find("TPC") != std::string::npos) {
    ...
    throw cet::exception("PDSPTPCInterface_tool")
  ...
  if (_enforce_no_duplicate_channels) {
    ...
    for (const auto& rd : raw_digits) {
      ...
      if (channels_seen.find(ichan) == channels_seen.end()) { channels_seen.insert(ichan); }
      else {
        MF_LOG_WARNING("PDSPTPCDataInterface:")
  ...
  if (_enforce_median_tick_count) {
    ...
    size_t tickmed = 0;
    if (tls != 0) { tickmed = TMath::Median(tls, ticklist.data()); }
    ...
    for (size_t i = 0; i < raw_digits.size(); ++i) {
      if (ticklist.at(i) != tickmed) {
        ...
        if ((crate == 3) && (slot == 3) && (fiber == 2) &&
            (ticklist.at(i) > 0.9 * tickmed && ticklist.at(i) < tickmed))
          continue; // FEMB 302
        dlist.push_back(i);
    ...
    if (dlist.size() != 0) {
      ...
      MF_LOG_WARNING("PDSPTPCDataInterface_tool:")
        << " Discarding data with n_ticks not at the median. Example invalid ticks: "
        << tickexample;
  ...
  if (flagged_duplicate)
    statword =
      4; // a flag to the caller indicating that the entire event's worth of raw digits is to be dropped
  _collectRDStatus(rdstatuses);
  return statword;
}
                                       ...
                                       std::vector<int>& apalist)
{
  ...
  if (inputLabel.find("Container") != std::string::npos) {
    ...
    if (cont_frags) {
      have_data = true;
      if (!_rceProcContNCFrags(
            cont_frags, n_rce_frags, true, evt, raw_digits, timestamps, apalist)) {
        return false;
      }
    }
  }
  ...
  for (auto const& frag : *frags) {
    ...
    if (frag.sizeBytes() < _rce_frag_small_size) {
      if (_drop_small_rce_frags) {
        MF_LOG_WARNING("_process_RCE:") << " Small RCE fragment size: " << frag.sizeBytes()
...
bool PDSPTPCDataInterface::_process_RCE_AUX(art::Event& evt,
                                            ...
                                            std::vector<int>& apalist)
{
  ...
  if (_rce_hex_dump) {
    ...
    std::cout << "RCE Fragment: all numbers in hex " << std::hex
              << "   SequenceID = " << frag.sequenceID() << "   fragmentID = " << frag.fragmentID()
    ...
    for (size_t bcounter = 0; bcounter < dsize; ++bcounter) {
      if ((offcounter % 8) == 0) {
        std::cout << std::endl
                  << std::hex << std::setfill('0') << std::setw(8) << offcounter << " ";
      }
  ...
  art::ServiceHandle<dune::PdspChannelMapService> electronicsMap;
  ...
  if (_rce_save_frags_to_files) {
    ...
    std::cout << "Saved an RCE fragment with " << rce.size() << " streams: " << outfilename
              << std::endl;
  }

  artdaq::Fragment cfragloc(frag);
  size_t cdsize = cfragloc.dataSizeBytes();
  const uint64_t* cdptr =
    (uint64_t const*)(cfragloc.dataBeginBytes() + 12); // see dunepdlegacy/Overlays/RceFragment.cc
  ...
  if (!isOkay) {
    MF_LOG_WARNING("_process_RCE_AUX:")
      << "RCE Fragment isOkay failed: " << cdsize << " Discarding this fragment";
    _DiscardedCorruptData = true;
    return false;
  }
  ...
  for (int i = 0; i < rce.size(); ++i) {
    ...
    for (size_t ialist = 0; ialist < apalist.size(); ++ialist) {
      if (((apalist[ialist] == -1) &&
           (!_rce_drop_frags_with_badc || (crateNumber > 0 && crateNumber < 7))) ||
    ...
    if (slotNumber > 4 || fiberNumber == 0 || fiberNumber > 4) {
      if (_rce_drop_frags_with_badsf) {
        MF_LOG_WARNING("_process_RCE:")
          << "Bad crate, slot, fiber number, discarding fragment on request: " << crateNumber << " "
    ...
    if (!_initialized_tick_count_this_event) {
      ...
      if (n_ticks != _tick_count_this_event) {
        if (_enforce_same_tick_count) {
          MF_LOG_WARNING("_process_RCE_AUX:")
            << "Nticks different for two channel streams: " << n_ticks << " vs "
            << _tick_count_this_event << " Discarding Data";
          _DiscardedCorruptData = true;
          return false;
        }
      }
    ...
    if (buffer_size > _rce_buffer_size_checklimit) {
      if (_rce_check_buffer_size) {
        MF_LOG_WARNING("_process_RCE_AUX:")
          << "n_ch*nticks too large: " << n_ch << " * " << n_ticks << " = " << buffer_size
          << " larger than: " << _rce_buffer_size_checklimit << ".  Discarding this fragment";
        _DiscardedCorruptData = true;
        return false;
      }
      else {
    ...
    if (!sgmcdretcode) {
      if (_enforce_error_free) {
        MF_LOG_WARNING("_process_RCE_AUX:") << "getMutliChannelData returns error flag: "
                                            << " c:s:f:ich: " << crateNumber << " " << slotNumber
                                            << " " << fiberNumber << " Discarding Data";
        _DiscardedCorruptData = true;
        return false;
      }
      _KeptCorruptData = true;
    ...
    for (size_t i_ch = 0; i_ch < n_ch; i_ch++) {
      unsigned int offlineChannel = electronicsMap->GetOfflineNumberFromDetectorElements(
        crateloc, slotNumber, fiberNumber, i_ch, dune::PdspChannelMapService::kRCE);

      if (_max_offline_channel >= 0 && _min_offline_channel >= 0 &&
          _max_offline_channel >= _min_offline_channel &&
          (offlineChannel < (size_t)_min_offline_channel ||
           offlineChannel > (size_t)_max_offline_channel))
        continue;

      v_adc.clear();

      if (_rce_fix110 && crateNumber == 1 && slotNumber == 0 && fiberNumber == 1 &&
          electronicsMap->ChipFromOfflineChannel(offlineChannel) == 4 &&
          n_ticks > _rce_fix110_nticks) {
      ...
      // It has been observed that the last four ticks of FEMB 302's reported data are undefined.
      // That is, they vary from run to run of the same program in the same event, and they have
      // unsual values.  To keep the behavior of the other ticks constant, since dataprep stretches
      // the waveforms to the full size, is to fill in the last four ticks in this case with the
      // last value (tick 5991).  This is not fully general -- it does not address "long" readouts, or unusal
      // event sizes.  Only do this in the RCE decoder as it only affects FEMB 302

      auto uncompressed_nticks = v_adc.size();
      if (uncompressed_nticks == 5996) {
        for (size_t i = 5992; i < 5996; ++i) {
          v_adc[i] = v_adc[5991];
        }
      }
                                         ...
                                         std::vector<int>& apalist)
{
  ...
  if (inputLabel.find("Container") != std::string::npos) {
    ...
    if (cont_frags) {
      have_data = true;
      if (!_felixProcContNCFrags(
            cont_frags, n_felix_frags, true, evt, raw_digits, timestamps, apalist)) {
        return false;
      }
    }
  }
    ...
    if (frags) {
      have_data_nc = true;
      if (!_felixProcContNCFrags(
            frags, n_felix_frags, false, evt, raw_digits, timestamps, apalist)) {
        return false;
      }
    }
  }
  ...
  for (auto const& frag : *frags) {
    ...
    if (frag.sizeBytes() < _felix_frag_small_size) {
      if (_drop_small_felix_frags) {
        MF_LOG_WARNING("_process_FELIX:") << " Small FELIX fragment size: " << frag.sizeBytes()
                                          << " Discarding just this fragment on request.";
        _DiscardedCorruptData = true;
        process_flag = false;
      }
    ...
    if (process_flag) {
      if (is_container) {
        artdaq::ContainerFragment cont_frag(frag);
        for (size_t ii = 0; ii < cont_frag.block_count(); ++ii) {
          if (_process_FELIX_AUX(evt, *cont_frag[ii], raw_digits, timestamps, apalist))
            ++n_felix_frags;
        }
      }
      else {
                                              ...
                                              std::vector<int>& apalist)
{
  ...
  if (slot > 4) {
    if (_felix_drop_frags_with_badsf) {
      _DiscardedCorruptData = true;
      MF_LOG_WARNING("_process_FELIX_AUX:") << "Invalid crate or slot: c=" << (int)crate
                                            << " s=" << (int)slot << " discarding FELIX data.";
      return false;
    }
    _KeptCorruptData = true;
  }
  ...
  for (size_t ialist = 0; ialist < apalist.size(); ++ialist) {
    if (((apalist[ialist] == -1) && (!_rce_drop_frags_with_badc || (crate > 0 && crate < 7))) ||
        (apalist[ialist] == (int)crate) || (apalist[ialist] == 7 && (crate == 0 || crate > 6))) {
      foundapainlist = true;
      break;
    }
  }
  ...
  const unsigned n_frames =
    felix
      .total_frames(); // One frame contains 25 felix (20 ns-long) ticks.  A "frame" is an offline tick
  ...
  if (n_frames * n_channels > _felix_buffer_size_checklimit) {
    if (_felix_check_buffer_size) {
      MF_LOG_WARNING("_process_FELIX_AUX:")
        << "n_channels*n_frames too large: " << n_channels << " * " << n_frames << " = "
        << n_frames * n_channels << " larger than: " << _felix_buffer_size_checklimit
  ...
  for (unsigned int iframe = 0; iframe < n_frames; ++iframe) {
    if (felix.wib_errors(iframe) != 0) {
      if (_enforce_error_free) {
        _DiscardedCorruptData = true;
        MF_LOG_WARNING("_process_FELIX_AUX:") << "WIB Errors on frame: " << iframe << " : "
                                              << felix.wib_errors(iframe) << " Discarding Data";
  ...
  for (unsigned ch = 0; ch < n_channels; ++ch) {
    ...
    if (fiber == 1) { fiberloc = 1; }
    else if (fiber == 2) {
      fiberloc = 3;
    }
    else {
      MF_LOG_WARNING("_process_FELIX_AUX:")
        << " Fiber number " << (int)fiber << " is expected to be 1 or 2 -- revisit logic";
      fiberloc = 1;
      if (_felix_drop_frags_with_badsf) {
    ...
    unsigned int offlineChannel = electronicsMap->GetOfflineNumberFromDetectorElements(
      crateloc, slot, fiberloc, chloc, dune::PdspChannelMapService::kFELIX);
    ...
    if (!_initialized_tick_count_this_event) {
      ...
      if (_enforce_same_tick_count) {
        if (v_adc.size() != _tick_count_this_event) {
          MF_LOG_WARNING("_process_FELIX_AUX:")
            << "Nticks different for two channel streams: " << v_adc.size() << " vs "
            << _tick_count_this_event << " Discarding Data";
          _DiscardedCorruptData = true;
          return true;
        }
...
void PDSPTPCDataInterface::computeMedianSigma(raw::RawDigit::ADCvector_t& v_adc,
                                              float& median,
                                              float& sigma)
{
