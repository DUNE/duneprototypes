////////////////////////////////////////////////////////////////////////
// Class:       T0RecoAnodePiercersVD
// Module Type: producer
// File:        T0RecoAnodePiercersVD.cc
//
...
class T0RecoAnodePiercersVD : public art::EDProducer {
  	public:
    	explicit T0RecoAnodePiercersVD(fhicl::ParameterSet const & p);
    	// The destructor generated by the compiler is fine for classes
    	// without bare pointers or other resource use.

    	// Plugins should not be copied or assigned.
    	T0RecoAnodePiercersVD(T0RecoAnodePiercersVD const &) = delete;
    	T0RecoAnodePiercersVD(T0RecoAnodePiercersVD &&) = delete;
    	T0RecoAnodePiercersVD & operator = (T0RecoAnodePiercersVD const &) = delete;
    	T0RecoAnodePiercersVD & operator = (T0RecoAnodePiercersVD &&) = delete;
...
T0RecoAnodePiercersVD::T0RecoAnodePiercersVD(fhicl::ParameterSet const & fcl)
  	:
    EDProducer(fcl) {
	...
	det_back   = fEdgeWidth;
		...
		if(fDebug) std::cout << "TPC drift distance: " << TPC.DriftDistance() << std::endl;
		if(TPC.DriftDistance() < 15.0) continue;

                auto const center = TPC.GetCenter();
                
		if(fDebug) std::cout << "TPC center: (" << center.X()       << "," << center.Y()       << "," << center.Z()       << ")" << std::endl;
		if(fDebug) std::cout << "TPC half:   (" << TPC.HalfWidth() << "," << TPC.HalfHeight() << "," << TPC.HalfLength() << ")" << std::endl;
		...
		if (tpc_top 	> det_right) 	det_right    = tpc_top;
		if (tpc_bottom 	< det_left) 	det_left = tpc_bottom;
...
void T0RecoAnodePiercersVD::produce(art::Event& event){
	...
	if(!MC){
		...
		if(fDebug) std::cout << fTriggerProducer << " trigger time: " << trigger_time << std::endl;
	}
	...
	for(unsigned int particle = 0; particle < reco_particles_h->size(); ++particle){
		...
		for (auto& hits : hit_v) {
			...
			if(fDebug) std::cout << "\t\t\t" << hits->Channel() << " " << hit_time << std::endl;
		...
		if (rc_ys < (det_right - fEdgeWidth) && rc_zs > (det_front + fEdgeWidth) 
		...
		if (rc_ye > (det_left + fEdgeWidth) && rc_ze > (det_front + fEdgeWidth) 
		...
		if(!anode_piercing_candidate) {
			if(fDebug) std::cout << "\t\tTrack does not pierce anode." << std::endl;
                        if(fDebug) std::cout << "Geometry cuts: " << rc_ye << " " << (det_left + fEdgeWidth) << " - " << rc_ze << " " << (det_front + fEdgeWidth) << " - " << rc_ze << " " << (det_back - fEdgeWidth) << std::endl;
                        if(fDebug) std::cout << "Timing cuts:   " << rc_xe << " " << rc_xs << " - " << driftDir_end << " - " << rc_xe << " " << rc_xs << " - " << driftDir_end << std::endl;
			continue; 
			}
		...
		*/
		...
		// CREATE T0 OBJECT AND ASSIGN TO PFPARTICLE
                ...
                if(fDebug) std::cout << "\t\t dt cut: " << length << " " << fMinTrackLength << " - " << matched_flash_pe << " " << fMinPE << " - " << dt_flash_reco << " [" << dtMin << "," << dtMax << "]" << std::endl;
...
void	T0RecoAnodePiercersVD::SortTrackPoints(const recob::Track& track, std::vector<TVector3>& sorted_trk)
{
	sorted_trk.clear();

	TVector3 track_start, track_end;	
	double start_y = det_left  - fEdgeWidth;
	double end_y   = det_right + fEdgeWidth;
...
size_t  T0RecoAnodePiercersVD::FlashMatch(const double reco_time, std::vector<double> op_times_v)
{
...
DEFINE_ART_MODULE(T0RecoAnodePiercersVD) 
